...
import type { LocationType } from '~/shared/components/filter-bar/types'
import { isIndoorSession } from '~/wrap-up/components/wrap-up/_components/training-summary/helpers'
import { PERIOD_TO_DATES, type Period } from '~/shared/schema'
import type { LoadCategory, TrainingSession } from '~/training/schema.ts'

type OptionalTrainingInput = Partial<Omit<TrainingSession, 'date' | 'comments' | 'load'>> & {
  year?: number
  load?: LoadCategory
  period?: Period
...
export function filterTrainingSessions(
  ...
  return trainingSessions.filter(trainingSession => {
    ...
    return (
      ...
      (load === undefined || isLoadInLoadCategory(trainingSession.load, load)) &&
      (sessionType === undefined || trainingSession.sessionType === sessionType) &&
...
function isLoadInLoadCategory(load: TrainingSession['load'], loadCategory: LoadCategory): boolean {
