import { _GRADES, type Ascent, type Grade, gradeSchema } from '~/schema/ascent'
...
export function createGradeScaleFromAscents(ascents: Ascent[]): Grade[] {
  if (ascents.length === 0) return _GRADES.map(g => gradeSchema.parse(g))
  ...
  return _GRADES.slice(lowerGradeIndex, higherGradeIndex + 1).map(g => gradeSchema.parse(g))
}
