...
- Regenerate responsive loading skeletons: start the development server, then run
  `vp run skeletons:build` in another terminal

Loading skeleton geometry is generated by Boneyard from the development-only
`/skeleton-catalog` route and committed under `src/bones/`. Regenerate it whenever
the catalog fixtures or their responsive layout changes.
